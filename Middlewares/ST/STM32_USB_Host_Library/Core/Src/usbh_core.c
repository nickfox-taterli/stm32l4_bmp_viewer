...
USBH_StatusTypeDef  USBH_Init(USBH_HandleTypeDef *phost, void (*pUsrFunc)(USBH_HandleTypeDef *phost, uint8_t ), uint8_t id)
{
  ...
  /*Create USB Host Task */
    xTaskCreate((TaskFunction_t)USBH_Process_OS, "USBH_Thread", USBH_PROCESS_STACK_SIZE, ( void * ) phost, 1, phost->thread);
#endif  
...
#if (USBH_USE_OS == 1)
      xQueueSend ( phost->os_event, (void *)USBH_PORT_EVENT, 0);
#endif  
  return USBH_OK;  
}
...
#if (USBH_USE_OS == 1)
        xQueueSend ( phost->os_event, (void *)USBH_STATE_CHANGED_EVENT, 0);
#endif         
      }
    }
    break;
...
#if (USBH_USE_OS == 1)
  xQueueSendFromISR(phost->os_event, (void *)USBH_PORT_EVENT, &taskWoken);
...
static void USBH_Process_OS(void const * argument)
{
  ...
  for(;;)
  {
        if(xQueueReceive(((USBH_HandleTypeDef *)argument)->os_event, &event, portMAX_DELAY)  == pdTRUE)
        {
...
{
  portBASE_TYPE taskWoken = pdFALSE;
  xQueueSendFromISR(phost->os_event, (void *)USBH_URB_EVENT, &taskWoken);	
  portEND_SWITCHING_ISR(taskWoken);
  return USBH_OK;
}
#endif  
